...
*更新：2026-03-26 | 维护角色：@documentor | 状态：🟢active*
...
| REQ-004 | 退出/掉线重连（30秒窗口） | 🟢待测试 | `@test_writer` | `docs/01-PRODUCT/PRD.md` §5、§6.4；`docs/02-ARCHITECTURE/ARCHITECTURE.md` §14；`docs/02-ARCHITECTURE/API_SPEC.md` §3.5、§4、§6.4；`docs/04-TESTING/TEST_STRATEGY.md` §13 | `@feature_dev`已补 Android 真机 socketerror 诊断增强：debug 连接模式支持 `polling+websocket` / `websocket-only` 切换，登录与工作区补齐稳定状态文本和测试标识；现等待 `@test_writer` 复跑真机与自动化，确认可诊断连接与 REQ-004 恢复链路。 |
...
- `REQ-004`已回到验证阶段：`@feature_dev`补齐 Android 真机 socketerror 诊断增强与 transport 可切换策略，当前状态更新为`🟢待测试 / @test_writer`；待 `@test_writer` 复跑真机与自动化后再决定是否推进到收尾归档。
